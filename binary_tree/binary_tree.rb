...
class Tree
  ...
  def build_tree(tree)
    ...
    center = tree.length/2
    node = Node.new(tree[center])
    node.left = build_tree(tree[0...center])
    node.right = build_tree(tree[center + 1..-1])
    node
  end
  ...
  def find_parent(data, node = @root)
    if node.left.nil? && node.right.nil?
      ...
      node.left.data == data ? node : find_pareent(data, node.left)
    end
  end
